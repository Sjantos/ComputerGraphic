...
namespace GKProject
{
    class SBlock : BaseBlock
    {
        ...
        public SBlock(int startWidthIndex, ref GameBoardCell[,] gameBoard, int bW, MyCubeTexture texture) : base(startWidthIndex - 2, ref gameBoard, bW, texture)
        {
            actualPosition = Position.Horizontal;
            usedCells.Add(0, new Tuple<int, int>(startWidthIndex - 1, boardHeight - 2));//Block indexes layout
            usedCells.Add(1, new Tuple<int, int>(startWidthIndex    , boardHeight - 1));//         1 3
            usedCells.Add(2, new Tuple<int, int>(startWidthIndex    , boardHeight - 2));//       0 2
            usedCells.Add(3, new Tuple<int, int>(startWidthIndex + 1, boardHeight - 1));
